...
async function generateViaOpenAICompatible(opts: LlmRequestOptions): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 32,
      messages: [
        { role: 'system', content: 'You are a playful Swedish Stockholm icon. Reply with joyful onomatopoeia, Swedish slang, or location shoutouts, and include exactly one emoji.' },
...
async function generateViaOllama(opts: LlmRequestOptions): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      model: opts.model,
      prompt: `${opts.prompt}\nRespond as a cheerful Stockholm symbol with Swedish vibes, mixing playful sound effects and keeping exactly one emoji.`,
      stream: false,
      options: { temperature: 0.95, num_predict: 32 }
    }),
    signal: opts.signal
  });
