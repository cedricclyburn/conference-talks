  ...
  return (
    <div className="absolute top-2 left-2 right-2 md:left-4 md:right-auto z-20 bg-white/70 backdrop-blur rounded px-3 py-2 text-sm shadow">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-4">
        <div className="font-semibold tracking-wide">Stockholm Signals</div>
        <div>Icons afloat: <span className="font-medium">{metrics.totalCreatures}</span></div>
        <div>Inflight: <span className="font-medium">{metrics.inflight}</span></div>
        <div>Avg fika-time: <span className="font-medium">{metrics.avgLatencyMs} ms</span></div>
        <div>Oopsies: <span className="font-medium text-red-600">{metrics.errors}</span></div>
      </div>
