...
# Deploy Blackjack AI Game with Ollama local model
# This script deploys the blackjack demo using local Ollama via Llama Stack
...
echo "🎮 Deploying Blackjack AI Game with Ollama local model..."
...
oc label namespace "$NAMESPACE" purpose=ai-gaming-demo --overwrite

# Deploy using kustomize with ollama overlay
echo "🚀 Deploying with Ollama configuration..."
oc apply -k "$REPO_ROOT/kubernetes/deploy-demo/overlays/ollama"
...
oc rollout status deployment/ramalama-deployment -n "$NAMESPACE" --timeout=300s
